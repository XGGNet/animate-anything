...
class EchoVideo(Dataset):
    def __init__(self, config, splits=["TRAIN", "VAL", "TEST"], chunks=1, chunk=0):
        ...
        # os.path.join(config.dataset.data_path, "Videos")

        # Transformations
        self.target_fps = config["dataset"].get("target_fps", 32)
        # config.dataset.get("fps", 32)
        self.duration = config["dataset"].get("duration", 2.0)
        # config.dataset.get("duration", 2.0)
        # self.videos_length = int(self.target_fps * self.duration)
        # TODO
        self.videos_length = 25
    ...
    def __getitem__(self, i):
        ...
        return {'pixel_values': video.permute(1,0,2,3), 'lvef': lvef, 'cond_frame': cond_frame, 'fname': fname}